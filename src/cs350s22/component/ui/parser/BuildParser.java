    ...
    public void parse() {
        ...
        if (args[index].equalsIgnoreCase("NETWORK")) {
            ...
            if (index < args.length && index + 1 == args.length) {
                components.add(parserHelper.getSymbolTableController().get(
                        Identifier.make(args[index])));
                index++;
                if (index < args.length && index + 1 == args.length) {
                    components.add(parserHelper.getSymbolTableActuator().get(
                            Identifier.make(args[index])));
                    index++;
                    if (index < args.length && index + 1 == args.length) {
                        components.add(parserHelper.getSymbolTableSensor().get(
                                Identifier.make(args[index])));
                    }
                }
            }
